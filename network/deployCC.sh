...
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --sequence 2 --tls --cafile $ORDERER_CA --output json


## commit the chaincode definition
infoln "commit the chaincode definition"

PEER_CONN_PARMS="--peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt 
...
## TEST1 : Invoking the chaincode : 킥보드 등록
infoln "TEST1 : Invoking the chaincode(Register)"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"RegisterKickboard","Args":["0001", "경북대학교", "2208181200"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3

## TEST2 : Invoking the chaincode : 킥보드 사용
infoln "TEST2 : Invoking the chaincode(Use)"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"UseKickboard","Args":["0001","0001","2208181500", "경북대학교 북문"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3

## TEST3 : Invoking the chaincode : 킥보드 사용종료
infoln "TEST3 : Invoking the chaincode(Finish)"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"FinishKickboard","Args":["0001","0001","2", "96", "2208181530", "경북대학교 정문"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
# ## TEST5 : Invoking the chaincode : 정기적 정보 등록
# infoln "TEST5 : Invoking the chaincode(Enroll)"
# set -x
# peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"EnrollData","Args":["0001", "94", "경북대학교 정문"]}' >&log.txt
# { set +x; } 2>/dev/null
# cat log.txt
# sleep 3


## TEST6 : Query the chaincode
infoln "TEST6 : Query the chaincode"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function":"QueryKickboard","Args":["0001"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt